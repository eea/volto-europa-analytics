...
if (typeof window.Matomo !== "object") {
    window.Matomo = window.Piwik = (function() {
                    ...
                    for (ax = 0; ax < L.length; ax++) {
                        if (x(aA)) {
                            ...
                            if (av[aA]) {
                                ...
                                var aC = "The method '" + aA + '\' was not found in "Matomo_paq" variable.  Please have a look at the Matomo tracker documentation: https://developer.matomo.org/api-reference/tracking-javascript';
                                ao(aC);
                                if (!aB) {
                    ...
                    if (ay[aw] && ay[aw][0]) {
                        ...
                        if (at === au) {
                            ...
                            if (az[au] > 1 && au !== "addTracker" && au !== "enableLinkTracking") {
                                ao("The method " + au + ' is registered more than once in "Matomo_paq" variable. Only the last call has an effect. Please have a look at the multiple Matomo trackers documentation: https://developer.matomo.org/guides/tracking-javascript-guide#multiple-piwik-trackers')
                            }
                            az[au]++
                        }
                    }
        ...
        function ah(av, au) {
            var at = new T(av,au);
            L.push(at);
            Matomo_paq = c(Matomo_paq, E);
            for (H = 0; H < Matomo_paq.length; H++) {
                if (Matomo_paq[H]) {
                    aj(Matomo_paq[H])
                }
            }
...
(function() {
    function b() {
        if ("object" !== typeof Matomo_paq) {
            return false
        }
        var c = typeof Matomo_paq.length;
        if ("undefined" === c) {
            return false
        }
        return !!Matomo_paq.length
    }
            ...
            Matomo_paq = {
                push: function(c) {
                    ...
                    if (d !== "undefined" && console && console.error) {
                        console.error("Matomo_paq.push() was used but Matomo tracker was not initialized before the matomo.js file was loaded. Make sure to configure the tracker via Matomo_paq.push before loading matomo.js. Alternatively, you can create a tracker via Matomo.addTracker() manually and then use Matomo_paq.push but it may not fully work as tracker methods may not be executed in the correct order.", c)
                    }
                }
            }
        }
